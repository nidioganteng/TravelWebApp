<div class="w-full md:w-80 bg-white border border-gray-200 rounded-3xl p-6 flex flex-col min-h-150 shadow-sm">
    <div class="flex items-center gap-4 mb-6 pb-6 border-b border-gray-100">
        <div class="w-16 h-16 rounded-full bg-gray-100 flex items-center justify-center text-xl font-bold text-gray-400 shadow-inner">
            {{ collect(explode(' ', auth()->user()->name))->map(fn($n) => ucfirst(substr($n, 0, 1)))->join('') }}
        </div>
        <div>
            <h3 class="font-extrabold text-gray-900 text-lg leading-tight">{{ auth()->user()->name }}</h3>
    ...
    <form method="POST" action="{{ route('logout') }}" class="mt-auto pt-6">
        @csrf
        <button type="submit" class="flex items-center gap-3 px-4 py-2 text-gray-900 font-bold hover:text-red-500 transition">
